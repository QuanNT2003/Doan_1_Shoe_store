    ...
    <div className=" bg-cyan-600 text-white transition-all">
      <div className="flex justify-around py-2 items-center md:w-[85%] md:mx-auto mt-2 h-[130px]">
        <div className=" flex items-center lg:justify-center justify-start ssm:mx-5">
          <img
            src={image}
            className="md:h-[90px] md:w-[90px] h-[60px] w-[60px] rounded-md"
          />
        </div>
        ...
        <div onClick={handleOpen} className="lg:hidden cursor-pointer me-2 ">
          <FontAwesomeIcon
            icon={faBars}
            className="hover:scale-110 me-4 w-[25px] h-[25px]"
          />
        </div>
        <div className="justify-end items-center hidden lg:flex flex-1">
          <div className="me-5 hover:cursor-pointer group relative">
            <div className="flex justify-center items-center group-hover:scale-110">
              <FontAwesomeIcon
                icon={faBell}
                className="me-4 w-[25px] h-[25px]"
              />
              <div className="text-[13px] hidden md:block">Thông báo</div>
            </div>
                  ...
                  <div>
                    {notifi.map((item, index) => (
                      <div key={index} className="border-b">
                        <div className="text-black h-[70px]  text-wrap text-[13px] font-medium p-1 mt-3">
                          {item.note}{" "}
                        </div>
                        <div className="h-[20px] text-black text-[12px]">
                          {now.diff(item.createdAt, "days") > 1
                            ? format(
                                new Date(item.createdAt),
                                "dd MMM yyyy - HH:mm"
                              )
                            : now.diff(item.createdAt, "hours") +
                              " giờ trước"}{" "}
                        </div>
                      </div>
                    ))}
          ...
          <div
            className="me-5 hover:cursor-pointer group relative "
          ...
          >
            <div className="flex justify-center items-center group-hover:scale-110">
              ...
              <div className="text-[13px] hidden md:block">Giỏ hàng</div>
            </div>
          </div>
          {window.localStorage.getItem("role") === "user" ? (
                  ...
                  <div className="flex items-center gap-3 text-black">
                    {user === "" ? (
                      ...
                      <Avatar
                        alt="Remy Sharp"
                        src={
                          user?.images[0]?.url ? user?.images[0]?.url : example
                        }
                      />
                    )}
      ...
      <div className="my-7 lg:flex">
        <div className="lg:flex hidden mx-auto h-[100%]">
          <NavLink
            ...
            className={({ isActive }) =>
              isActive
                ? "max-w-[240px] min-w-[200px] text-black bg-white h-[50px] rounded-t-lg flex justify-center items-center "
                : "max-w-[240px] min-w-[200px] h-[50px] flex justify-center items-center"
            }
          >
