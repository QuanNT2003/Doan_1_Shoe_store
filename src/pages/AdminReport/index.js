...
function SellReport() {
  ...
  const createObjectQuery = async (startDate, endDate, groupBy) => {
    return {
      startDate,
      endDate,
      groupBy,
  ...
  const handleGetMoney = async (obj) => {
    ...
    const responseMoney = await reportServices.GetMoney(obj).catch((error) => {
      console.log(error);
  ...
  const callTopSell = async (value) => {
    const fetch = async () => {
      ...
      if (value === 1) {
        const responseTopProducts = await reportServices
          .GetTopProduct({
        ...
        <div className=" md:flex items-center p-3">
          ...
          <div className="md:ms-6 mt-4">
            <button
              ...
              onClick={() => callGetRevenues()}
            >
              Xem báo cáo
            </button>
          </div>
          {loadingMoney && (
            <CircularProgress className="max-w-[200px] ml-5" color="primary" />
          )}
        </div>
        <div className="md:flex">
          <div className="md:w-[50%] flex flex-col items-center">
            <div>Tổng doanh thu</div>
            <div className="text-[25px] font-medium text-blue-600 mt-3">
              {addCommas(totalRevenue)} đ
            </div>
          </div>
          <div className="md:w-[50%] flex flex-col items-center">
            <div>Tổng nhập hàng</div>
            <div className="text-[25px] font-medium text-pink-600 mt-3">
              {addCommas(totalCost)} đ
            </div>
          </div>
        </div>
