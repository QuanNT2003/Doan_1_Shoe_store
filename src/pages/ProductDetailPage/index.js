...
function ProductPage() {
  ...
  const getVersion = async (sizeId, colorId) => {
    const fetchApi = async () => {
      ...
      if (result) {
        ...
        if (result.data[0].inStock < quantity)
          toastContext.notify(
            "warning",
            "Số lượng sản phẩm còn dưới " + quantity
          );
      }
    };
  ...
  const addToCart = async () => {
    ...
    const fetchApi = async () => {
      ...
      const result = await ShoppingCartServices.CreateShoppingCart(
        cartItem
      ).catch((error) => {
        console.log(error);
        setLoading(false);
  ...
  useEffect(() => {
    const fetchApi = async () => {
      ...
      const result = await ProductServices.getProduct(productID.id).catch(
        (err) => {
          console.log(err);
        }
      );
      ...
      const resultSize = await VersionServices.getVersionSize(
        productID.id
      ).catch((err) => {
        console.log(err);
      });
      ...
      if (resultSize) {
        const data = await resultSize.data.map((cate) => ({
          name: cate.name,
          value: cate,
        }));
        setSizeList(data);
      }
      const resultColor = await VersionServices.getVersionColor(
        productID.id
      ).catch((err) => {
        console.log(err);
      });
      ...
      if (resultColor) {
        const data = await resultColor.data.map((cate) => ({
          name: cate.name,
          value: cate,
        }));
        setColorList(data);
        ...
        <div className="select-none">
          <div className="bg-white m-5 mb-10 p-5 rounded-lg lg:grid lg:grid-cols-5">
            ...
            <div className="p-4 pl-6 lg:col-span-3">
              ...
              <div className="text-[25px] text-red-500 mb-2">
                {addCommas((obj.price * (100 - obj.discount)) / 100)} đ
              </div>
              <div className="">
                <span className="me-3 line-through text-stone-400">
                  {addCommas(obj.price)} đ
                </span>
              ...
              <div>
                ...
                <div className="flex mt-5 items-center text-gray-500">
                  ...
                  <div className="flex">
                    <div
                      className="w-[40px] h-[35px] flex justify-center items-center bg-stone-300 text-gray-500 rounded mx-3 cursor-pointer"
                      onClick={() => {
                    ...
                    >
                      <div
                        className={
                          quantity <= 1
                            ? "bg-stone-100 w-[100%] h-[100%] flex justify-center items-center text-gray-300 cursor-not-allowed"
                            : ""
                        }
                      >
                    ...
                    <input
                      className="border-custom number-nospin rounded w-[50px] py-1 px-[10px] text-[14px] text-center hover:border-blue-500"
                      value={quantity}
                      onChange={(e) => {
                ...
                {version?.inStock < 10 ? (
                  <div className="flex justify-center mt-5 text-[17px] font-semibold">
                    Còn {version?.inStock} sản phẩm{" "}
                  </div>
                ) : (
                  <div> </div>
                )}
              ...
              <div className="mt-7 ssm:flex ssm:flex-row">
                <div className="ssm:w-[50%] mb-2 ssm:mb-0 flex justify-center items-center">
                  <button
                    className="bg-amber-500 py-5 px-3 rounded-lg w-[80%] text-white hover:bg-amber-400 cursor-pointer "
                    onClick={() => handleOpen()}
                  >
                    Mua Ngay
                  </button>
                ...
                <div className="ssm:w-[50%] flex justify-center items-center">
                  <button
                    className="bg-orange-500 py-5 px-3 rounded-lg w-[80%] text-white hover:bg-orange-400 cursor-pointer "
                    onClick={() => {
                      addToCart();
                    }}
                  >
          ...
          <div className="bg-white m-5 mb-10 p-5 rounded-lg">
            <div className="font-bold mb-3 text-[17px]">Mô tả sản phẩm</div>
            <div
              className={
                showall === true
                  ? "text-[14px] mb-10 transition-all"
                  : "transition-all text-[14px] mb-10 line-clamp-4"
              }
            >
              {obj.description}
            </div>
            {obj.description === "" ? (
              ...
              <div>
                ...
                >
                  <button
                    className="bg-blue-500 mx-auto p-3 rounded-lg w-[30%] text-white hover:bg-blue-400 cursor-pointer "
                ...
                >
                  <button
                    className="bg-orange-500 mx-auto p-3 rounded-lg w-[30%] text-white hover:bg-orange-400 cursor-pointer "
