...
function HomePage() {
  ...
  const getCate = async () => {
    const response = await CategoryServices.getAllCategorys().catch((error) => {
      if (error.response) {
  ...
  const getList = async (obj, setList, setPending) => {
    setPending(true);
    const response = await ProductServices.getAllProducts(obj).catch(
      (error) => {
        setPending(false);
  ...
  useEffect(() => {
    const fetch = async () => {
      getList(
        await createObjectQuery(1, 12, "discount", "desc"),
        setListSale,
        setPendingSale
      );
      getList(await createObjectQuery(1, 12), setListNew, setPendingNew);
      getList(
        await createObjectQuery(1, 20),
        setListJustForYou,
        setPendingJustForYou
      );
      const now = new Date();
  ...
  return (
    <div className="select-none">
      <ProductCarousel
        ...
        path="/collection/desc&discount"
      />
      <Categories list={listCategories} />
      <ProductCarousel
        title={"Sản phẩm mới"}
        listProduct={listNew}
        path="/collection/desc&creatAt"
      />
      <ProductCarousel
